#CS2302, Fernando De Santiago, LAB1, Olac Fuentes, Anindita Nath and Maliheh Zargaran, last edited 2/8/19 9:20:00 PM, Section M/W 10:30-11:50
"""
...
def recur_tree(ax, x, y, width, height, tiers):
	if tiers > 0:
		...
		tiers -= 1
		recur_tree(ax, x, y+height, width/2, height, tiers) #moving the tree down slowly
		recur_tree(ax, x+width/2, y+height, width/2, height, tiers)#mocing the tree in different directions slowly

def tree(ax, x, y, width, height, tiers):
	tier_height = height/tiers #how much each tier goes down
	recur_tree(ax, x, y, width, tier_height, tiers)
	
fig, ax6=plt.subplots()
tree(plt,0,0,100,-100,3) #plot starting at [0,0] and going to [100,-100] and doing it 3 times each time getting smaller in tier but lower down
ax6.set_aspect(1.0)
plt.show()
fig.savefig('tree.png')
fig, ax7=plt.subplots()
tree(plt,0,0,100,-100,4)#plot starting at [0,0] and going to [100,-100] and doing it 4 times each time getting smaller in tier but lower down
ax7.set_aspect(1.0)
plt.show()
fig.savefig('tree2.png')
fig, ax8=plt.subplots()
tree(plt,0,0,100,-100,5)#plot starting at [0,0] and going to [100,-100] and doing it 5 times each time getting smaller in tier but lower down
ax8.set_aspect(1.0)
plt.show()
